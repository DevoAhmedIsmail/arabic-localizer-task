...
const CardList = ({searchText,pageNumber,numOfCard,addOptions,openModalHandler}) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 md:gap-x-[20px] lg:gap-x-[15px] xl:gap-x-[30px] gap-y-[20px] xl:gap-y-[35px]">
        {employees.company_users?.data.length ? (
          employees.company_users.data.map((data) => (
            <CardItem data={data} key={data.id} searchText={searchText} pageNumber={pageNumber} numOfCard={numOfCard} openModalHandler={openModalHandler} addOptions={addOptions} />
          ))
        ) : (
